package com.csws.mymaps.coordinators.ui;
...
import com.csws.mymaps.activities.map.controllers.BottomSheetController;
import com.csws.mymaps.activities.map.controllers.MapFabController;
import com.csws.mymaps.activities.map.controllers.MapToolbarController;
import com.csws.mymaps.activities.map.controllers.TopSheetController;
import com.csws.mymaps.activities.map.fragments.bottom_sheets.DayPlanFragment;
import com.csws.mymaps.activities.map.fragments.bottom_sheets.LocationPlanFragment;
...
import com.csws.mymaps.activities.map.models.DefaultBrowsingViewModel;
import com.csws.mymaps.activities.map.behaviour.MapBrowsingMode;
...
public class MapViewCoordinator  implements MapToolbarController.Listener,MapFabController.FabActionListener,MapFragment.MapCallbacks, TopSheetController.Listener, BottomSheetController.Listener, UiCoordinator, BrowsingActions {
    ...
    public void observe(LifecycleOwner owner) {
        ...
        coordinatorContext.sessionManager.getLocationsLiveData().observe(owner, locations -> {
        ...
        coordinatorContext.sessionManager.getTasksLiveData().observe(owner, refreshTasks);
        coordinatorContext.sessionManager.getPlansLiveData().observe(owner, refreshTasks);

        Log.d(TAG, "observe END");
    }
    private void refreshTaskMarkers() {

        List<TaskItem> tasks = coordinatorContext.sessionManager.getAllTasks();
        List<PlannedTask> plans = coordinatorContext.sessionManager.getAllPlans();
    ...
    @Override public void onBackPressed() {
        Workflow workflow = coordinatorContext.workflowManager.getActiveWorkflow();
        if (workflow != null) {
    ...
    public void showLocationDetails(LocationItem location) {
        Log.d("MapViewCoordinator", "LocationDetails Requested");

        List<PlannedTask> plans = coordinatorContext.sessionManager.getPlansForLocation(location.id);

        List<TaskItem> tasks = coordinatorContext.sessionManager.getTasksForLocation(location.id);
    ...
    public void showDayPlan() {
        ...
        DailySession session = coordinatorContext.sessionManager.getCurrentSession();
        List<TaskItem> tasks = coordinatorContext.sessionManager.getAllTasks();
        List<PlannedTask> plans = coordinatorContext.sessionManager.getAllPlans();
        DayPlanFragment dayPlanFragment = DayPlanFragment.newInstance(session, tasks, plans);

        PlannerState plannerState = coordinatorContext.promptResultListener.getCurrentState();
    ...
    public void startCreateLocation() {
        coordinatorContext.workflowManager.startCreateLocationFlow();
    }

    @Override
    public void startCreateTask() {
        coordinatorContext.workflowManager.startCreateTaskFlow();
    }

    @Override
    public void startCreatePlan() {
        coordinatorContext.workflowManager.startCreatePlannedTaskFlow();
    }

    @Override
    public void startCreateCollection() {
        coordinatorContext.workflowManager.startCreateCollectionFlow();
    }

    @Override
    public void startCreateTaskFromLocation(LocationItem location) {
        coordinatorContext.workflowManager.startCreateTaskFromLocationFlow(location);
    }
    ...
    private void tryDisplayNextPrompt() {
        ...
        if (coordinatorContext.workflowManager.hasActiveWorkflow()) {
            return;
        }
    ...
    private void displayPrompt(PlannerPrompt prompt) {
        ...
        if (prompt.type ==
            ...
            LocationItem destination = coordinatorContext.sessionManager.getLocationById(destinationId);

            if (destination != null) {

                LatLng userLatLng = mapController.getUserLocation();
                // request route preview
                coordinatorContext.routeService.calculateRoute(
                        userLatLng,
        ...
        fragment.setListener(
                new PlannerPromptFragment.Listener() {
                    ...
                    public void onPromptResult(PlannerPromptResult result) {
                        ...
                        coordinatorContext.promptResultListener.submitPromptResult(result);
